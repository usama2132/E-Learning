...
const Sidebar = ({ isOpen, onClose, isMobile, isTablet, isDesktop, theme }) => {
  ...
  const getSidebarContainerStyles = useMemo(() => {
    ...
    return {
      ...
      transform: 'translateX(0)', // Always in position when displayed
      transition: 'none',
      ...
      return (
        <div key={item.key}>
          <button
            style={{
              ...styles.navItem,
              ...(isActive || hasActiveSubmenu ? styles.navItemActive : {}),
            }}
            onClick={() => toggleSubmenu(item.key)}
            onMouseEnter={(e) => {
              if (!isActive && !hasActiveSubmenu && !isMobile) {
                e.target.style.background = isDark ? 'rgba(71, 85, 105, 0.5)' : 'rgba(226, 232, 240, 0.5)';
              }
            }}
            onMouseLeave={(e) => {
              if (!isActive && !hasActiveSubmenu && !isMobile) {
                e.target.style.background = (isActive || hasActiveSubmenu) ? 
                  (isDark ? 'rgba(99, 102, 241, 0.2)' : 'rgba(99, 102, 241, 0.1)') : 'transparent';
              }
            }}
          ...
          <div style={{
            ...
            {item.submenu.map((subItem, index) => (
              <Link
                ...
                onClick={onClose}
                onMouseEnter={(e) => {
                  if (!isActiveRoute(subItem.path) && !isMobile) {
                    e.target.style.background = isDark ? 'rgba(71, 85, 105, 0.3)' : 'rgba(226, 232, 240, 0.3)';
                  }
                }}
                onMouseLeave={(e) => {
                  if (!isActiveRoute(subItem.path) && !isMobile) {
                    e.target.style.background = isActiveRoute(subItem.path) ? 
                      (isDark ? 'rgba(99, 102, 241, 0.15)' : 'rgba(99, 102, 241, 0.08)') : 'transparent';
                  }
                }}
              >
                {subItem.label}
          ...
          <div style={styles.sidebarFooter}>
            <div style={styles.footerActions}>
              ...
              <button
                ...
                onMouseEnter={(e) => {
                  e.target.style.background = isDark ? 'rgba(239, 68, 68, 0.2)' : 'rgba(239, 68, 68, 0.1)';
