...
const useProgress = (courseId = null) => {
  ...
  // Ensure every enrolled course has a default progress object
const safeCourseProgress = {};
if (courseProgress) {
  Object.keys(courseProgress).forEach(courseId => {
    safeCourseProgress[courseId] = courseProgress[courseId] || {
      completedLessons: [],
      currentLesson: { lessonId: null, sectionId: null, position: 0 },
      overallPercentage: 0,
      totalTimeSpent: 0,
      lastWatchedAt: null,
      totalLessons: 0,
  ...
  return {
    currentProgress,
    courseProgress: safeCourseProgress,
    loading,
    error,
    fetchCourseProgress,
    fetchAllProgress,
