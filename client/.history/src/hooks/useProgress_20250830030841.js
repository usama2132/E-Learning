...
const useProgress = (courseId = null) => {
  ...
  const requestBody = {
    courseId: targetCourseId,  // Add courseId field
    lessonId: lessonId,
    // Remove other fields that might not be expected by validation schema
  };
    ...
    try {
      ...
      // Ensure proper data structure for validation
      const requestBody = {
        lessonId: lessonId,
      ...
      const response = await makeAPIRequest(`/progress/lesson`, {  // Changed endpoint
  ...
  const getUserCourses = useCallback(async (userId) => {
    try {
      ...
      // Don't set error for empty courses - return empty array
