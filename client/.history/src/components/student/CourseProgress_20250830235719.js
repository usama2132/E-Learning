...
const CourseProgress = () => {
  ...
  const fetchCourseAndProgress = async () => {
    ...
    try {
      ...
      // FIXED: Check enrollment first using correct backend endpoint
      ...
      // FIXED: Fetch course details using correct backend endpoint
      ...
      // FIXED: Handle course structure properly from backend
      if (!courseInfo.sections || courseInfo.sections.length === 0) {
        // Create sections from existing data if needed
        const sections = [];
        ...
        if (courseInfo.previewVideo?.url) {
          sections.push({
            ...
            lessons: [
              {
                id: courseInfo.previewVideo.public_id || 'intro-lesson',
        ...
        // Add actual course sections if they exist
        if (courseInfo.sections && courseInfo.sections.length > 0) {
          sections.push(...courseInfo.sections);
        }

        courseInfo.sections = sections;
      }

      // Ensure proper structure for lessons
      courseInfo.sections = courseInfo.sections.map(section => ({
        ...section,
        lessons: (section.lessons || []).map(lesson => ({
          ...lesson,
          id: lesson._id || lesson.id || `lesson-${Date.now()}`,
          thumbnail: lesson.thumbnail || courseInfo.thumbnail?.url || courseInfo.thumbnail || '',
          videoUrl: lesson.videoUrl || lesson.videos?.[0]?.url || '',
          description: lesson.description || lesson.title || '',
          type: lesson.type || (lesson.videos?.length > 0 ? 'video' : 'content')
        }))
      }));

      setCourse(courseInfo);

      // FIXED: Fetch progress using correct backend endpoint
      try {
  ...
  const handleLessonClick = (lesson, sectionIndex, lessonIndex) => {
    if (lesson.videoUrl || lesson.type === 'video') {
      // FIXED: Navigate to correct video lesson route
  ...
  const markLessonCompleted = async (lessonId) => {
    try {
      ...
      // FIXED: Use correct backend endpoint for marking lesson complete
      ...
      if (response.ok) {
        ...
        console.error('Failed to mark lesson complete:', response.status);
      }
    } catch (error) {
