...
const CourseProgress = () => {
  ...
  const fetchCourseAndProgress = async () => {
    ...
    try {
      ...
      // FIXED: Check if user is actually enrolled in this course
      ...
      // Ensure course has proper structure for display
      if (!courseInfo.sections || courseInfo.sections.length === 0) {
        // Create sections from course data if available
        const sections = [];
      ...
      courseInfo.sections = courseInfo.sections.map(section => ({
        ...
        lessons: (section.lessons || []).map(lesson => ({
          ...
          videoUrl: lesson.videoUrl || lesson.video?.url || lesson.videos?.[0]?.url || '',
          description: lesson.description || lesson.title || '',
          type: lesson.type || (lesson.videoUrl || lesson.videos?.length > 0 ? 'video' : 'content')
        }))
      }));

      setCourse(courseInfo);

      // FIXED: Fetch progress using correct endpoint from backend routes
      try {
  ...
  const markLessonCompleted = async (lessonId) => {
    try {
      ...
      // FIXED: Use correct endpoint for marking lesson complete
