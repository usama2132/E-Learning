  ...
  const fetchCourseProgress = async () => {
    try {
      ...
      if (courseResponse.ok) {
        ...
        setCourse(courseData.data?.course || courseData.course);
        
        // Set first lesson if available
        if (courseData.data?.course?.sections?.[0]?.lessons?.[0]) {
          setCurrentLesson(courseData.data.course.sections[0].lessons[0]);
        }
      }
