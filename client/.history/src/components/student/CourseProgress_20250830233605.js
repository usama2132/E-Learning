...
const CourseProgress = () => {
  ...
  const fetchCourseAndProgress = async () => {
    ...
    try {
      ...
      // Fetch course details with sections and videos
      ...
      // FIXED: Get actual course content from instructor uploads
      try {
        ...
        if (contentResponse.ok) {
          ...
          if (contentData.success && contentData.data) {
            // Merge actual uploaded content
            courseInfo.sections = contentData.data.sections || courseInfo.sections;
            courseInfo.videos = contentData.data.videos || [];
          }
        }
      } catch (contentError) {
        console.warn('Could not fetch course content:', contentError);
      }

      // FIXED: If no sections but has videos, create sections from videos
      if ((!courseInfo.sections || courseInfo.sections.length === 0) && courseInfo.videos && courseInfo.videos.length > 0) {
        courseInfo.sections = [{
          id: 'main-content',
          title: 'Course Videos',
          description: 'Main course content',
          lessons: courseInfo.videos.map(video => ({
            ...
            thumbnail: video.thumbnail || courseInfo.thumbnail?.url || courseInfo.thumbnail,
            isPreview: false
          }))
        }];
      }
      ...
      courseInfo.sections = (courseInfo.sections || []).map(section => ({
        ...
        lessons: (section.lessons || []).map(lesson => ({
          ...
          videoUrl: lesson.videoUrl || lesson.video?.url || '',
