...
const MyLearning = () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.courses) {
            // FIX: Ensure every course has `id` for consistent mapping
  ...
  // Memoized computation for filtering and stats
  ...
  return (
    <div className={`my-learning ${themeClass}`}>
      ...
      <div className="my-learning-controls">
        <SearchBar placeholder="Search your courses..." value={searchTerm} onChange={setSearchTerm} />
        <CourseFilters filters={filterOptions} activeFilter={filterType} onFilterChange={setFilterType} />
      </div>

      <div className="my-learning-stats">
        <div className="stat-card"><h3>{stats.totalCourses}</h3><p>Total Courses</p></div>
        <div className="stat-card"><h3>{stats.coursesStarted}</h3><p>Courses Started</p></div>
        <div className="stat-card"><h3>{stats.coursesCompleted}</h3><p>Courses Completed</p></div>
        <div className="stat-card"><h3>{stats.totalLessons}</h3><p>Total Lessons</p></div>
      </div>
      ...
      <div className="courses-grid">
        {filteredCourses.length === 0 ? (
          <div className="no-courses">
            ...
            <p>{enrolledCourses.length === 0 ? "You haven't enrolled in any courses yet." : "No courses match your current filters."}</p>
            {enrolledCourses.length === 0 && <Link to="/courses" className="btn btn-primary">Browse Courses</Link>}
          </div>
        ) : (
            ...
            <div key={course.id} className="course-card">
              <div className="course-image">
                ...
                <div className="course-status">
                  <span className={`status-badge ${course.status?.toLowerCase().replace(' ', '-') || ''}`}>{course.status || 'Not Started'}</span>
                </div>
              </div>

              <div className="course-content">
                <h3>{course.title}</h3>
                <p className="instructor">By {course.instructor?.name || 'Unknown'}</p>
                <ProgressBar completed={course.completedCount || 0} total={course.totalLessons || 0} showPercentage={true} />
                ...
                <div className="course-actions">
                  <Link to={`/course-progress/${course.id}`} className="btn btn-primary">
      ...
      {filteredCourses.length > 0 && <div className="my-learning-footer"><p>Keep up the great work! 🎉</p></div>}
    </div>
  );
};
