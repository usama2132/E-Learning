...
import { Play, Pause, SkipBack, SkipForward, Volume2, Settings, Maximize, FileText, MessageSquare, BookOpen, Star, Moon, Sun } from 'lucide-react';
...
const VideoLecture = () => {
  ...
  const { isDarkMode, toggleTheme, getCurrentColors, actualTheme } = useTheme();
...
useEffect(() => {
...
}, [isFullscreen, controlsTimeoutId]);
  ...
  // FIXED: Throttled progress tracking to prevent multiple API calls
  useEffect(() => {
    let progressInterval;
    
    if (isPlaying && videoRef.current && duration > 0 && !isUpdatingProgress) {
      progressInterval = setInterval(() => {
        ...
        if (current > 0 && total > 0) {
          ...
          // FIXED: Only update progress every 15 seconds and if significant change
          const now = Date.now();
          if (now - lastUpdateTime.current > 15000 && Math.abs(percentage - watchedPercentage) >= 5) {
            updateVideoProgressThrottled(current, total, percentage);
          }
        }
  ...
  }, [isPlaying, duration, watchedPercentage, isUpdatingProgress]);

  // FIXED: Save progress on unmount with final update
  useEffect(() => {
    return () => {
      if (currentTime > 0 && duration > 0 && videoRef.current) {
        const finalPercentage = Math.round((currentTime / duration) * 100);
        updateVideoProgressFinal(currentTime, duration, finalPercentage);
      }
    };
  ...
  // FIXED: Throttled progress update to prevent spam
  const updateVideoProgressThrottled = async (currentTime, duration, percentage) => {
    ...
    // Set timeout to prevent rapid updates
    progressUpdateTimeout.current = setTimeout(async () => {
      try {
        await updateVideoProgressAPI(currentTime, duration, percentage);
      } finally {
  ...
  // FIXED: Final progress update on unmount
  const updateVideoProgressFinal = async (currentTime, duration, percentage) => {
    try {
      await updateVideoProgressAPI(currentTime, duration, percentage);
    } catch (error) {
  ...
  // FIXED: Core API function with better error handling
  const updateVideoProgressAPI = async (currentTime, duration, percentage) => {
    if (!lessonId || !courseId || duration === 0) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (percentage >= 90 && watchedPercentage < 90) {
  ...
  const videoContainer = document.querySelector('.video-lecture .video-container');
  if (!isFullscreen) {
  ...
  if (!lesson || (!lesson.videoUrl && !lesson.videos?.length)) {
    return (
      <div className={`video-lecture-error ${actualTheme === 'dark' ? 'dark-mode' : 'light-mode'}`}>
  ...
  return (
    <div className={`video-lecture ${actualTheme === 'dark' ? 'dark-mode' : 'light-mode'}`}>
        ...
        <div className="video-container">
          <video
            ...
            onEnded={() => {
              ...
              updateVideoProgressAPI(duration, duration, 100);
            ...
            <div className="video-controls">
              ...
              <div className="controls-row">
                <div className="left-controls">
                  ...
                  <button onClick={() => handleSeek(Math.max(0, currentTime - 10))} className="control-btn">
                    <SkipBack size={20} />
                  </button>
                  <button onClick={() => handleSeek(Math.min(duration, currentTime + 10))} className="control-btn">
                    <SkipForward size={20} />
                  </button>
        ...
        <div className="lecture-info">
          <div className="lecture-header">
            ...
            <div className="lecture-status">
              <span className={`status-badge ${watchedPercentage >= 90 ? 'completed' : 'in-progress'}`}>
                {watchedPercentage >= 90 ? 'Completed' : 'In Progress'}
              </span>
              {isUpdatingProgress && <span className="saving-indicator">Saving...</span>}
            </div>
          </div>
          ...
          <div className="lecture-meta">
            ...
            <div className="meta-item">
              ...
              <span className="value">{formatTime(lesson.duration || duration)}</span>
            </div>
          </div>
          
          <div className="progress-display">
            <div className="progress-info">
              <span className="progress-text">Progress: {watchedPercentage}%</span>
            </div>
            <ProgressBar 
          ...
          {activeTab === 'notes' && (
            <div className="notes-section">
              <div className="note-editor">
                ...
                />
                <Button onClick={handleSaveNote} className="primary small save-note-btn">
                  Save Note
                </Button>
              </div>
                ...
                {savedNotes.map(note => (
                  <div key={note.id} className="note-item">
                    <div className="note-timestamp">
                      <button onClick={() => handleSeek(note.timestamp)} className="timestamp-btn">
