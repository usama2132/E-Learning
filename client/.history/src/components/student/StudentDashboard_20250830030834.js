...
const StudentDashboard = () => {
  ...
  const fetchEnrolledCourses = async () => {
    try {
      ...
      if (error.message.includes('404') || error.message.includes('No enrolled courses')) {
        return []; // Return empty array for 404 or no courses
      }
  ...
  const fetchProgress = async (courseIds) => {
    ...
    try {
      // Use the progress endpoint that matches your backend routes
      const response = await makeAPIRequest('/student/progress');
      ...
      if (response.success && response.data) {
        ...
        if (response.data.progress && Array.isArray(response.data.progress)) {
          response.data.progress.forEach(progress => {
            progressMap[progress.courseId || progress.course] = progress;
          });
        }
        return progressMap;
      }
  ...
  const fetchRecommendedCourses = async () => {
    try {
      // Use courses endpoint with proper query parameters
      const response = await makeAPIRequest('/courses?featured=true&limit=4');
