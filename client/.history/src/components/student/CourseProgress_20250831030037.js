...
// CourseProgress.js - Replace fetchCourseAndProgress function (lines 27-115)
...
const fetchCourseAndProgress = async () => {
  ...
  try {
    ...
    // FIXED: Get videos from backend Video collection
    const videosResponse = await fetch(`http://localhost:5000/api/videos/course/${courseId}`, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
      },
    ...
    if (videosResponse.ok) {
      ...
      console.log('Videos fetch failed, status:', videosResponse.status);
      const errorText = await videosResponse.text();
      console.log('Videos error response:', errorText);
    }

    // Build sections from actual Video collection data
    if (videos.length > 0) {
      const sectionsMap = {};
      ...
      console.log('No videos found - using fallback content');
      courseInfo.sections = [{
        id: 'empty-section',
