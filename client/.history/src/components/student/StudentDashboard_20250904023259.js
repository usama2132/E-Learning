...
const StudentDashboard = () => {
  ...
  // Fetch enrolled courses with correct endpoint
    ...
    try {
      ...
      if (response.success && response.courses) {
        return response.courses.map(course => ({
          ...
          totalLessons: course.sections?.reduce((total, section) => total + (section.lessons?.length || 0), 0) || course.totalVideos || 1,
          // Get completed lessons count
          completedLessons: course.progress?.completedLessons?.length || 0,
          // Calculate progress percentage
          progressPercentage: course.progress?.overallPercentage || 0,
          // Ensure progress object exists
          progress: {
            overallPercentage: course.progress?.overallPercentage || 0,
            completedLessons: course.progress?.completedLessons || [],
            totalTimeSpent: course.progress?.totalTimeSpent || 0,
            lastAccessedAt: course.progress?.lastAccessedAt || course.enrollmentDate || new Date(),
            ...course.progress
          }
        }));
      }
  ...
  // Get progress percentage for a course - FIXED
  const getProgressPercentage = useCallback((course) => {
    // First check if progress percentage is directly available
    if (course.progressPercentage !== undefined) {
      return Math.round(course.progressPercentage);
    }
    ...
    // Calculate from completed lessons vs total lessons
    const completedLessons = course.completedLessons || course.progress?.completedLessons?.length || 0;
    const totalLessons = course.totalLessons || 1;
    
    return Math.round((completedLessons / totalLessons) * 100);
  }, []);

  // Check if course is in progress - FIXED
  const isCourseInProgress = useCallback((course) => {
    const progressPercentage = getProgressPercentage(course);
    return progressPercentage > 0 && progressPercentage < 100;
  }, [getProgressPercentage]);

  // Check if course is completed - FIXED
  const isCourseCompleted = useCallback((course) => {
    const progressPercentage = getProgressPercentage(course);
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        const [coursesResult, recommendedResult, statsResult] = await Promise.allSettled([
        ...
        if (recommendedResult.status === 'fulfilled') {
          console.log('🎯 Recommended courses loaded:', recommendedResult.value.length);
          setRecommendedCourses(recommendedResult.value);
        }
  ...
  // Calculate processed stats from data - FIXED
  const processedStats = useMemo(() => {
    ...
    const completedCourses = enrolledCourses.filter(course => isCourseCompleted(course)).length;
    const inProgressCourses = enrolledCourses.filter(course => isCourseInProgress(course)).length;
  ...
  }, [enrolledCourses, stats, isCourseCompleted, isCourseInProgress]);
  ...
  // Filter courses in progress - FIXED
  const coursesInProgress = useMemo(() => {
    return enrolledCourses.filter(course => isCourseInProgress(course));
  }, [enrolledCourses, isCourseInProgress]);

  // Filter active courses (courses with any progress) - FIXED
  const activeCourses = useMemo(() => {
    return enrolledCourses.filter(course => {
      const progressPercentage = getProgressPercentage(course);
      return progressPercentage > 0;
    });
  }, [enrolledCourses, getProgressPercentage]);
    ...
    <div className="student-dashboard">
      ...
      <div className="stats-grid">
        <StatCard
          ...
          value={processedStats.totalCourses}
          icon="book"
          color="blue"
        />
        <StatCard
          title="Completed Courses"
          value={processedStats.completedCourses}
          icon="check-circle"
          color="green"
        />
        <StatCard
          title="In Progress"
          value={processedStats.inProgressCourses}
          icon="play-circle"
          color="orange"
        />
        <StatCard
          title="Hours Learned"
          value={processedStats.totalHoursLearned}
          icon="clock"
          color="purple"
        />
      </div>
