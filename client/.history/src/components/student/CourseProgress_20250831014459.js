...
const CourseProgress = () => {
  ...
  // Update the fetchCourseAndProgress function in CourseProgress.js
// Replace lines 27-115 with this fixed version:
...
const fetchCourseAndProgress = async () => {
  ...
  try {
    ...
    // FIXED: Fetch videos from backend API
    const videosResponse = await fetch(`http://localhost:5000/api/courses/${courseId}`, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
      },
    ...
    if (!videosResponse.ok) {
      throw new Error(`Failed to fetch course videos: ${videosResponse.status}`);
    }

    const courseData = await videosResponse.json();
    ...
    // FIXED: Get videos directly from Video collection via backend
    const videosFetchResponse = await fetch(`http://localhost:5000/api/videos/course/${courseId}`, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
      },
    ...
    if (videosFetchResponse.ok) {
      const videosData = await videosFetchResponse.json();
      if (videosData.success && videosData.data && videosData.data.videos) {
        videos = videosData.data.videos;
        console.log(`Found ${videos.length} videos from Video collection`);
      }
    } else {
      console.log('Direct videos fetch failed, using course sections');
    }

    // FIXED: Build sections from actual Video collection data
    if (videos.length > 0) {
      const sectionsMap = {};
      
      videos.forEach((video) => {
        const sectionKey = `section-${video.sectionNumber || 1}`;
        
        if (!sectionsMap[sectionKey]) {
          sectionsMap[sectionKey] = {
            id: sectionKey,
            title: `Section ${video.sectionNumber || 1}`,
            description: '',
            lessons: []
          };
        }
        ...
        const lesson = {
          ...
          thumbnail: video.thumbnailUrl || courseInfo.thumbnail?.url || '',
          isPreview: video.settings?.isPreview || false,
          resources: video.resources || [],
          quiz: video.quiz?.hasQuiz ? video.quiz : null
        };
        
        sectionsMap[sectionKey].lessons.push(lesson);
      });
      
      // Convert to array and sort
      courseInfo.sections = Object.values(sectionsMap).sort((a, b) => {
      ...
      courseInfo.sections.forEach(section => {
        section.lessons.sort((a, b) => a.orderIndex - b.orderIndex);
      });
      
      console.log(`Processed ${courseInfo.sections.length} sections with ${videos.length} total videos`);
    } else {
      // Fallback: use existing course sections if available
      if (courseInfo.sections && courseInfo.sections.length > 0) {
        console.log('Using existing course sections:', courseInfo.sections.length);
      } else {
        console.log('No videos or sections found');
        courseInfo.sections = [{
          id: 'empty-section',
    ...
    try {
      ...
      if (progressResponse.ok) {
        ...
        if (progressData.success && progressData.data) {
          setProgress(progressData.data.progress || progressData.data);
          
          const completed = progressData.data.progress?.completedLessons || 
                          progressData.data.progress?.lessons?.filter(l => l.completed).map(l => l.lessonId) ||
                          [];
