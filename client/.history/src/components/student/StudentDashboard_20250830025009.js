...
const fetchingRef = useRef(false); // Add this
  ...
  const fetchProgress = async (courseIds) => {
    ...
    try {
      ...
      const progressPromises = courseIds.map(async (courseId) => {
        try {
          const response = await fetch(`http://localhost:5000/api/progress/${user.id}/${courseId}`, {
            method: 'GET',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
          ...
          return { courseId, progress: { completedLessons: [], lastAccessed: null } };
        } catch (error) {
