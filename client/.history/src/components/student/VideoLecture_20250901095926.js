...
const VideoLecture = () => {
  ...
  // FIXED: Updated video progress function with correct endpoint
  const updateVideoProgress = async (currentTime, duration) => {
    try {
      const percentage = duration > 0 ? (currentTime / duration) * 100 : 0;
      
      // Update context progress first
      if (updateProgress) {
      ...
      // FIXED: Use the correct backend endpoint that exists in your progressController.js
      const token = localStorage.getItem('token') || localStorage.getItem('lms_auth_token');
      ...
      if (response.ok) {
        ...
        console.log('Backend progress updated successfully:', result);
        ...
        console.error('Failed to update backend progress:', errorData);
        
        // Don't throw error to avoid disrupting video playback
        // addNotification('Progress sync failed', 'warning');
      }
      
    } catch (error) {
      console.error('Failed to update progress:', error);
      // Don't disrupt video playback for progress sync errors
    }
  };
    ...
    <div className="video-lecture">
      <div className="video-section">
        ...
        <div className="lecture-info">
          ...
          {/* Progress Debug Info (remove in production) */}
          <div style={{ fontSize: '12px', color: '#666', marginTop: '10px' }}>
            Debug: {watchedPercentage.toFixed(2)}% watched | {formatTime(currentTime)} / {formatTime(duration)}
          </div>
        </div>
      </div>
