...
import '../../styles/component/CourseProgress.css';
...
const CourseProgress = () => {
  ...
  const fetchCourseAndProgress = async () => {
    ...
    try {
      ...
      // FIXED: Fetch course details with proper error handling
      ...
      // FIXED: Process course sections and lessons properly for backend structure
      if (courseInfo.sections && courseInfo.sections.length > 0) {
        ...
        courseInfo.sections = courseInfo.sections.map((section, sectionIndex) => {
          ...
          return {
            ...
            lessons: (section.lessons || []).map((lesson, lessonIndex) => {
              ...
              // FIXED: Handle video data properly from backend structure
              let videoUrl = '';
              let thumbnail = '';
              let duration = lesson.duration || 0;
              
              // Check if lesson has videos array (backend structure)
              if (lesson.videos && lesson.videos.length > 0) {
                const video = lesson.videos[0]; // Get first video
              ...
              // Check if lesson has direct video properties
              ...
              // Check if lesson content is a video link
        ...
        // FIXED: Check for course-level videos or preview video
        const sections = [];
        ...
        // FIXED: Check for course curriculum/content structure
  ...
  const markLessonCompleted = async (lessonId) => {
    try {
      ...
      // FIXED: Use correct backend endpoint for marking completion
