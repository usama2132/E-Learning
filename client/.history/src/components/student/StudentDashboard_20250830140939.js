...
const StudentDashboard = () => {
  ...
  try {
    // CHANGE: Use the exact endpoint that matches your backend routes
    const response = await makeAPIRequest('/student/enrolled-courses');
    
    if (response.success && response.courses) {  // CHANGE: Check for 'courses' not 'data.courses'
 ...
 const fetchProgress = async (courseIds) => {
  ...
  try {
    // FIXED: Use correct courses endpoint that exists in backend
