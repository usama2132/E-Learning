...
const AdminProfile = () => {
  ...
  // Update these functions in your AdminProfile.js component:
...
const handleSave = useCallback(async (e) => {
  ...
  try {
    const response = await fetch('http://localhost:5000/api/users/profile', {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token') || localStorage.getItem('lms_auth_token')}`,
    ...
    if (response.ok && data.success) {
      ...
      console.error('Failed to update admin profile:', data.message);
...
// Replace the handlePasswordUpdate function
