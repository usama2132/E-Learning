...
const InstructorDashboard = () => {
  ...
  // keep your custom hook for fetching courses if needed
  const { loading, getInstructorCourses } = useCourses(); 

  // ✅ add local state for courses
...
useEffect(() => {
  if (user?._id) {
    console.log('Fetching instructor courses for user ID:', user._id);
    courseAPI.getInstructorCourses(user._id)
      .then((res) => {
        const data = res.data || [];
        setCourses(data);
        setStats({
          totalCourses: data.length,
          totalStudents: data.reduce((sum, c) => sum + (c.students?.length || 0), 0),
          totalEarnings: data.reduce((sum, c) => sum + (c.earnings || 0), 0),
          pendingApproval: data.filter((c) => c.status === 'pending').length
        });
      })
