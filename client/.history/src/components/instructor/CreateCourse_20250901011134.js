...
const CreateCourse = () => {
  ...
  const stepLabels = ['Basic Info', 'Details', 'Media', 'Submit'];
  ...
  const validateStep = (step) => {
    ...
    switch (step) {
      ...
      case 3:
        ...
        break;
      case 4:
        if (!courseCreated) newErrors.submit = 'Course must be created first';
        const pendingVideos = videos.filter(v => !v.uploaded);
        if (pendingVideos.length > 0) newErrors.submit = 'All videos must be uploaded before submission';
        break;
    }
  ...
  const handleCreateCourse = async () => {
    ...
    try {
      ...
      if (thumbnail) {
        ...
        try {
          const thumbnailUrl = await uploadThumbnailToBackend(createdCourseId);
      ...
      // Move to next step for video uploads
      setCurrentStep(4);
  ...
  // Step 2: Submit course after videos uploaded
  const handleSubmitCourse = async (status = 'pending') => {
    ...
    try {
      ...
      await makeAuthenticatedRequest(`http://localhost:5000/api/courses/${realCourseId}`, {
        method: 'PUT',
      ...
      setErrors({ 
        submit: error.message || 'Failed to submit course. Please try again.' 
      });
    } finally {
      setUploading(false);
    }
      ...
      case 4:
        return (
          <div className="create-course__form-section">
            ...
            {courseCreated && (
              <div className="create-course__success-card">
                <div className="create-course__success-icon">✅</div>
                <p>Course created successfully! Course ID: {realCourseId}</p>
              </div>
            )}
            ...
            <div className="create-course__upload-status">
              <h4>Upload Status:</h4>
              <div className="create-course__status-grid">
                <div className="create-course__status-item">
                  <span className="create-course__status-label">Course:</span>
                  <span className={`create-course__status-value ${courseCreated ? 'create-course__status--success' : 'create-course__status--pending'}`}>
                    {courseCreated ? '✅ Created' : '⏳ Pending'}
                  </span>
                </div>
                <div className="create-course__status-item">
                  <span className="create-course__status-label">Videos:</span>
                  <span className={`create-course__status-value ${videos.every(v => v.uploaded) ? 'create-course__status--success' : 'create-course__status--pending'}`}>
          ...
          <div className="create-course__button-group">
            {currentStep < 3 ? (
              ...
              <>
                <button
                  type="button"
                  className={`create-course__btn create-course__btn--outline ${isLoading || !videos.every(v => v.uploaded) ? 'create-course__btn--disabled' : ''}`}
                  onClick={() => handleSubmitCourse('draft')}
                  disabled={isLoading || !videos.every(v => v.uploaded)}
                >
                  {isLoading ? (
                    <>
                      <div className="create-course__spinner"></div>
                ...
                <button
                  type="button"
                  className={`create-course__btn create-course__btn--primary ${isLoading || !videos.every(v => v.uploaded) ? 'create-course__btn--disabled' : ''}`}
                  onClick={() => handleSubmitCourse('pending')}
                  disabled={isLoading || !videos.every(v => v.uploaded)}
                >
                  {isLoading ? (
                    <>
