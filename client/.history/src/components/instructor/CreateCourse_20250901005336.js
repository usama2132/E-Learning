...
const CreateCourse = () => {
  ...
  const createCourseFirst = async () => {
  try {
    console.log('Creating course first...');
    setUploading(true);
    setErrors({});
  ...
  const uploadPendingVideos = async (courseId) => {
    ...
    const uploadPromises = pendingVideos.map(async (video, index) => {
      try {
        ...
        // Add required metadata with fallback values
  ...
  if (validateStep(currentStep)) {
    if (currentStep === 2 && !realCourseId) {
      try {
        await createCourseFirst();
        setCurrentStep(3);
      } catch (error) {
  ...
  const handleSubmit = async (status = 'draft') => {
    ...
    try {
      ...
      // Build course data that matches your backend Course model
      const courseData = {
        title: formData.title.trim(),
        description: formData.description.trim(),
        shortDescription: formData.shortDescription.trim(),
      ...
      // Step 3: Upload all pending videos
      if (videos.length > 0) {
      ...
      // Step 4: Update course with media URLs if needed
      if (thumbnailUrl) {
        try {
          const updateData = { 
            thumbnail: {
      ...
      // Success - navigate with appropriate message
  ...
  <div style={{
    ...
    Please create the course first by clicking "Next" on the previous step.
  </div>
)}
  ...
  disabled={isLoading}
>
  {currentStep === 2 && !realCourseId ? (
    isLoading ? 'Creating Course...' : 'Create Course & Continue →'
  ) : (
    'Next →'
  )}
</button>
