...
const VideoUploadManager = ({ courseId, onVideoAdded, onClose }) => {
  ...
  // Step 2: Create Lesson (you'll need section ID from previous step)
  const createLesson = async () => {
    const sectionId = prompt('Enter section ID (from previous step):');
    const lessonTitle = prompt('Enter lesson title:');
    if (!sectionId || !lessonTitle) return;
  ...
  // Step 3: Add Video to Lesson
  const addVideoToLesson = async () => {
    ...
    try {
      ...
      // Add video to lesson (using placeholder URL for now)
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '600px' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <button onClick={createSection} disabled={loading} style={{ marginRight: '10px' }}>
          {loading ? 'Creating...' : '1. Create Section'}
        </button>
        <button onClick={createLesson} disabled={loading} style={{ marginRight: '10px' }}>
          {loading ? 'Creating...' : '2. Create Lesson'}
        </button>
      </div>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        />
        <button onClick={addVideoToLesson} disabled={loading || !videoDetails.title.trim()}>
          {loading ? 'Adding...' : 'Add Video to Lesson'}
        </button>
      </div>

      <p style={{ color: '#666', fontSize: '14px' }}>
        Instructions:
        1. Click "Create Section" first
        2. Click "Create Lesson" (enter section ID from step 1)
        3. Enter video title and click "Add Video to Lesson"
      </p>
...
const EditCourse = () => {
  ...
  onClick={async () => {
    ...
    try {
      ...
      const result = await response.json();
      if (response.ok) {
        alert('Section created! Now create lesson and add video through the API.');
        fetchCourse(); // Refresh to show new section
      } else {
