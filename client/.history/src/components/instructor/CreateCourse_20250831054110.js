...
const CreateCourse = () => {
  ...
  const createSectionForVideos = async (courseId) => {
    try {
      console.log('📁 Creating default section for videos');
      
      const sectionResponse = await makeAuthenticatedRequest(`http://localhost:5000/api/courses/${courseId}/sections`, {
        method: 'POST',
        body: JSON.stringify({
          title: 'Course Content',
          description: 'Main course content and videos',
          order: 1
        })
      });
      ...
      const lessonResponse = await makeAuthenticatedRequest(`http://localhost:5000/api/courses/${courseId}/sections/${section._id}/lessons`, {
        method: 'POST',
        body: JSON.stringify({
          title: 'Introduction',
          description: 'Course introduction and main content',
          content: '',
          duration: 0,
          order: 1
        })
      ...
      return { sectionId: section._id, lessonId: lesson._id };

    } catch (error) {
      console.error('❌ Error creating section/lesson:', error);
      throw error;
    }
  };
  ...
  const uploadPendingVideos = async (courseId) => {
    ...
    try {
      const { sectionId, lessonId } = await createSectionForVideos(courseId);
      ...
      for (let i = 0; i < pendingVideos.length; i++) {
        ...
        if (data.success && data.data.video) {
          try {
            await makeAuthenticatedRequest(`http://localhost:5000/api/courses/${courseId}/sections/${sectionId}/lessons/${lessonId}/videos`, {
              method: 'POST',
              body: JSON.stringify({
                title: videoTitle,
                description: videoDescription,
                url: data.data.video.url,
                duration: data.data.video.duration || 0,
                thumbnail: data.data.video.thumbnail || {},
                isPreview: false,
                order: i + 1
              })
            });
            console.log('✅ Video linked to lesson successfully');
          } catch (linkError) {
            console.error('❌ Failed to link video to lesson:', linkError);
          }
        }
