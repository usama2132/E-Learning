...
const EditCourse = () => {
  ...
  // FIXED: Use correct backend route from instructor.js
  const addSectionToDatabase = async () => {
    ...
    try {
      setSaving(true);
      // CORRECTED: Backend route is /instructor/courses/:id/sections
      ...
      if (response.ok) {
        ...
        alert('Failed to create section: ' + (result.message || 'Unknown error'));
      }
    } catch (error) {
  ...
  const addLessonToSection = async (sectionId) => {
    ...
    try {
      setSaving(true);
      // CORRECTED: Backend route uses courseId parameter consistently
      ...
      if (response.ok) {
        ...
        alert('Failed to create lesson: ' + (result.message || 'Unknown error'));
      }
    } catch (error) {
  ...
  const addVideoToLesson = async (sectionId, lessonId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Failed to add video: ' + (result.message || 'Unknown error'));
      }
    } catch (error) {
