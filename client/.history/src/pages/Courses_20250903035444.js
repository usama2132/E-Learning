...
const Courses = () => {
  ...
  // Simplified filter state - only search and sort (removed CourseFilters)
  ...
  // Simplified initial load - fetch courses immediately
  useEffect(() => {
  ...
  return (
    <div className={`courses-page ${isDarkMode() ? 'dark' : 'light'}`}>
      ...
      <div className="courses-page__content courses-page__content--full-width">
        <main className="courses-page__main">
          {isLoading && hasInitialLoad ? (
            ...
            <>
              <div className="courses-page__grid courses-page__grid--three-columns">
                {courses.map(course => (
                  <CourseCard 
