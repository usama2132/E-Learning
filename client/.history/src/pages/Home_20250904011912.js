...
const Home = () => {
  ...
  const fetchHomePageCourses = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/courses?limit=50&status=published', {
        method: 'GET',
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          console.log(`Home: Loaded ${data.data.courses?.length || data.courses?.length || 0} courses`);
        } else {
  ...
  const processedCourses = useMemo(() => {
    ...
    // Featured courses (highest rated and approved)
    const featured = courses
      .filter(course => course.rating >= 4.0 && course.status === 'published')
    ...
    const popular = courses
      ...
      .sort((a, b) => (b.enrollmentCount || b.studentsCount || b.students || 0) - (a.enrollmentCount || a.studentsCount || a.students || 0))
      .slice(0, 6);
  ...
  const stats = useMemo(() => {
    ...
    return {
      totalCourses: courses.filter(course => course.status === 'published').length,
      totalStudents: courses.reduce((sum, course) => sum + (course.enrollmentCount || course.studentsCount || course.students || 0), 0),
      totalInstructors: new Set(courses.map(course => course.instructorId || course.instructor?._id || course.instructor)).size,
          ...
          <div className="hero-text">
            ...
            <div className="hero-actions">
              {!user ? (
                <>
                  ...
                  <Button onClick={handleBrowseCourses} variant="outline" className="secondary-cta">
                    Browse Courses
                  </Button>
                </>
              ) : (
                <>
                  <Button onClick={handleGoToDashboard} className="primary-cta">
                    <Book size={18} />
                    Go to Dashboard
                  </Button>
                  <Button onClick={handleBrowseCourses} variant="outline" className="secondary-cta">
            ...
            <div className="hero-trust-indicators">
              ...
              <div className="trust-item">
                ...
                <span>{stats.totalStudents}+ Happy Students</span>
              </div>
              <div className="trust-item">
      ...
      <section className="cta-section">
        <div className="cta-content">
          ...
          <div className="cta-buttons">
            {!user ? (
              <>
                <Button onClick={handleSignUp} className="cta-primary">
                  Sign Up Now
                </Button>
                <Button onClick={handleSignIn} variant="outline" className="cta-secondary">
                  Sign In
                </Button>
              </>
            ) : (
              <Button onClick={handleBrowseCourses} className="cta-primary">
                Browse All Courses
              </Button>
            )}
          </div>
