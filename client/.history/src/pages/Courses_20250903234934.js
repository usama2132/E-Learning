...
const Courses = () => {
  ...
  useEffect(() => {
    ...
    // CRITICAL FIX: Force initial fetch
    const initialParams = {
      page: 1,
      limit: 12,
      sortBy: 'newest'
  ...
  // CRITICAL FIX: Better loading and course display logic
  console.log('🎬 Render check:', {
    isLoading,
    hasInitialLoad,
    coursesExist: courses && courses.length > 0,
  ...
  // FIXED: Always render the page structure
    ...
    <div className={`courses-page ${isDarkMode() ? 'dark' : 'light'}`}>
      ...
      <div className="courses-page__content">
        ...
        <main className="courses-page__main">
          {/* FIXED: Show loading overlay while fetching but keep existing content visible */}
