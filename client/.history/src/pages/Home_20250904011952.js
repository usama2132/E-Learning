...
const Home = () => {
  ...
  // FIXED: Local state for Home page - completely isolated from CourseContext
  ...
  // FIXED: Direct API call to backend - bypasses CourseContext
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          console.log(`Home: Loaded ${data.data.courses?.length || 0} courses`);
        } else {
  ...
  // FIXED: Load courses only once on component mount
  useEffect(() => {
  ...
  const processedCourses = useMemo(() => {
    ...
    const popular = courses
      .sort((a, b) => (b.enrollmentCount || b.students || 0) - (a.enrollmentCount || a.students || 0))
      .slice(0, 6);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      totalStudents: courses.reduce((sum, course) => sum + (course.enrollmentCount || course.students || 0), 0),
      totalInstructors: new Set(courses.map(course => course.instructorId || course.instructor)).size,
