...
const handleResize = useCallback(() => {
  ...
  const wasMobileOrTablet = windowWidth <= 1024;
  const isNowDesktop = newWidth > 1024;
  const isNowMobileOrTablet = newWidth <= 1024;
  
  setWindowWidth(newWidth);
  
  // FIXED: Close sidebar when moving from mobile to desktop, keep closed on desktop to mobile
  if (wasMobileOrTablet && isNowDesktop) {
    // Moving from mobile/tablet to desktop - CLOSE and keep closed
    console.log('Resizing from mobile/tablet to desktop - closing sidebar');
    setSidebarOpen(false);
  } else if (wasDesktop && isNowMobileOrTablet) {
    // Moving from desktop to mobile/tablet - ALWAYS close sidebar
    console.log('Resizing from desktop to mobile/tablet - closing sidebar');
    setSidebarOpen(false);
  }
}, [windowWidth]);
...
useEffect(() => {
  ...
  const styles = {
    ...
    // In DashboardLayout.js, find the sidebarContainer styles and update to:
sidebarContainer: (() => {
  if (isDesktop) {
    // Desktop: Show sidebar when open, completely hide when closed
    return {
      position: 'relative',
